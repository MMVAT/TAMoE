...
def train(args, model, train_loader, optimizer, criterion, epoch, device):
    model.train()
    train_loss = {'total': 0, 'loss_video': 0, 'loss_valor_v': 0, 'loss_valor_a': 0, 'loss_all': 0, 'loss_o': 0}
    ...
    for batch_idx, batch_data in enumerate(train_loader):
        ...
        labels = batch_data['label'].float().to(device)  # ([64, 25])
        ...
        warm_up=1 if epoch<=args.warm_epoch else 0

        output, _, _, _, frame_logits, [loss_a_rec, loss_v_rec, loss_a_ort, loss_v_ort], [map_aa, map_av, map_vv, map_va], (out_1, out_2, out, update_flag, performance_1, performance_2), loss_moe \
            = model(audios, video_res_feats, video_3d_feats, label=labels, warm_up=warm_up)
        ...
        if warm_up==0 and args.use_adam_drop:
        ...
        loss_o=loss_fn(out,labels)
        loss_o1=loss_fn(out_1,labels)
        loss_o2=loss_fn(out_2,labels)
        total_loss_o+=loss_o.item()
        total_loss_o1+=loss_o1.item()
        total_loss_o2+=loss_o2.item()

        loss = loss_valor_a + loss_valor_v + loss_video + loss_rec + loss_ort * 0.1 + loss_ec * 0.1 + loss_o * 0.1 + loss_moe
        ...
        train_loss['loss_o'] += (loss_o.item() * batch_size)

        loss.backward()

        print(f"loss_o:  {loss_o.item():.4f} | "
            f"loss_o1: {loss_o1.item():.4f} | "
            f"loss_o2: {loss_o2.item():.4f} | "
            f"loss_valor_a: {loss_valor_a.item():.4f} | "
            f"loss_valor_v: {loss_valor_v.item():.4f} | "
            f"loss_video:   {loss_video.item():.4f} | "
            f"loss_rec:     {loss_rec.item():.4f} | "
            f"loss_ort:     {loss_ort.item():.4f} | "
            f"loss_ec:      {loss_ec.item():.4f}")
            

        if warm_up==0 and args.use_modulation:
            coeff_1, coeff_2 = None, None
            radio_1 = performance_1 / performance_2
            radio_2 = performance_2 / performance_1

            if args.use_OGM_plus:
                if radio_1 > 1:
                    coeff_2 = 4
                    coeff_1 = 1
                else:
                ...
                if radio_1 > 1:
                    coeff_1 = 1 - torch.tanh(args.alpha * torch.relu(torch.tensor(radio_1)))
                    coeff_2 = 1
                else:
                    coeff_1 = 1
                    coeff_2 = 1 - torch.tanh(args.alpha * torch.relu(torch.tensor(radio_2)))

            if args.modulation_starts <= epoch <= args.modulation_ends:
                for name, parms in model.named_parameters():
                    layer_name = str(name).split('.')[0]
                    if 'a_unmix' in layer_name and parms.grad is not None and len(parms.grad.size()) == 2:
                    ...
                    if 'v_unmix' in layer_name and parms.grad is not None and len(parms.grad.size()) == 2:
                        if args.modulation == 'OGM_GE':
                            parms.grad = parms.grad * coeff_2 + torch.zeros_like(parms.grad).normal_(0, parms.grad.std().item() + 1e-8)
                        elif args.modulation == 'OGM':
                            parms.grad *= coeff_2
...
def eval(args, model, data_loader, gt_csv_dir, criterion, device):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(data_loader):
            ...
            output, a_prob, v_prob, frame_prob, frame_logits = model(audios, video_res_feats, video_3d_feats, label=labels, mode='test')

            o = (output.cpu().detach().numpy() >= 0.5).astype(np.int_)  # (B, 25)
            ov = (v_prob.cpu().detach().numpy() >= 0.5).astype(np.int_)  # (B, 25)
            ...
            Pv = (Pv >= 0.5).astype(np.int_) * np.repeat(ov, repeats=10, axis=0)  # (10, 25)
            ...
            GT_av = GT_a * GT_v
            # Pa, Pv = set_multisource_true(Pa, Pv, GT_a, GT_v)
...
def main():
    ...
    parser.add_argument("--mode", type=str, default='train', choices=['train', 'val', 'test'], help="which mode to use")
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--epochs', type=int, default=60)
    parser.add_argument('--optimizer', type=str, default='adamw')
    parser.add_argument('--lr', type=float, default=1e-4)
    parser.add_argument('--grad_norm', type=float, default=1.0, help='the value for gradient clipping (0 means no gradient clipping)')
    ...
    parser.add_argument("--input_v_dim", type=int, default=2048)  # 768
    parser.add_argument("--input_a_dim", type=int, default=128)  # 768
    parser.add_argument("--hidden_dim", type=int, default=256)
    parser.add_argument("--nhead", type=int, default=8)
    parser.add_argument("--ff_dim", type=int, default=1024)
    parser.add_argument("--num_layers", type=int, default=4)
    parser.add_argument("--norm_where", type=str, default="post_norm", choices=['post_norm', 'pre_norm'])
    parser.add_argument("--model_name", type=str, help="the name for the model")
    parser.add_argument("--model_save_dir", type=str, default='models/', help="where to save the trained model")
    ...
    parser.add_argument('--use_modulation', action='store_true')
    parser.add_argument('--use_OGM_plus', action='store_true')
    parser.add_argument('--modulation_starts', type=int, default=0)
    parser.add_argument('--modulation_ends', type=int, default=20, help='epoch * 0.8/0.6')
    parser.add_argument('--modulation', type=str, default='OGM_GE', choices=['Normal', 'OGM', 'OGM_GE'])
    ...
    parser.add_argument('--q_base',type=float,default=0.8)
    parser.add_argument('--lam',type=float,default=0.5)
    parser.add_argument('--p_exe',type=float,default=0.1)
    parser.add_argument("-d", "--d", nargs="+", type=int, default=[512, 512], help="list of hidden dimensions, e.g. 512 512")

    # MOE
    parser.add_argument("--num_experts", type=int, default=16)
    parser.add_argument("--moe_nheads", type=int, default=8)
    parser.add_argument("--topK", type=int, default=4)
    parser.add_argument("--dropout", type=float, default=0.5)
    parser.add_argument("--fusion_sparse", type=bool, default=False) 
    ...
    parser.add_argument("--interaction_loss_weight", type=float, default=1e-1)
    ...
    if args.mode == 'train':
        ...
        best_F = {'Seg-type': 0.0, 'Seg-a': 0.0, 'Seg-v': 0.0, 'Avg': 0.0}
        best_epoch = 0
        for epoch in range(1, args.epochs + 1):
            ...
            logging.info(
                'Epoch[{}/{}](Time:{:.2f} sec)(lr:{:.6f}) Train Loss: {:.3f} Val F: {:.3f}'.format(epoch, args.epochs, elapse_time, cur_lr, train_loss_dict['loss_all'], F_scores['Avg']))
